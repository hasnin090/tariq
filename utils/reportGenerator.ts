 ...
 * محرك توليد التقارير المالية - يدعم PDF, Excel, Word
 */
...
<body>
    ...
    <p style="text-align: center; color: #64748b;">
        ${data.period}<br>
        تم الإنشاء في: ${formatDate(data.generatedAt)}<br>
        بواسطة: ${data.generatedBy}
    </p>
    
    <div class="summary">
        <h2>الملخص المالي</h2>
        <div class="summary-item"><strong>إجمالي الإيرادات:</strong> ${formatCurrency(data.totalRevenue)}</div>
        <div class="summary-item"><strong>إجمالي المصروفات:</strong> ${formatCurrency(data.totalExpenses)}</div>
        <div class="summary-item"><strong>صافي الربح:</strong> <span class="${data.netProfit >= 0 ? 'positive' : 'negative'}">${formatCurrency(data.netProfit)}</span></div>
        <div class="summary-item"><strong>هامش الربح:</strong> ${data.profitMargin.toFixed(2)}%</div>
    </div>
    ...
    <p style="text-align: center; color: #9ca3af; font-size: 12px;">
        تم إنشاء هذا التقرير تلقائياً من نظام إدارة المشاريع
    </p>
</body>
</html>
    `;
