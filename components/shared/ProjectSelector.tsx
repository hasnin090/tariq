...
const ProjectSelector: React.FC<ProjectSelectorProps> = ({ projects, activeProject, onSelectProject }) => {
    ...
    // If only one project, show it as a badge without selector
    if (projects.length === 1) {
        return (
            <div className="mb-6 bg-gradient-to-r from-primary-500 to-blue-600 rounded-xl p-4 shadow-lg">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-white/20 rounded-lg flex items-center justify-center backdrop-blur-sm">
                        <BriefcaseIcon className="h-5 w-5 text-white" />
                    </div>
                    <div>
                        <p className="text-xs text-white/80 font-medium">المشروع الحالي</p>
                        <p className="text-lg font-bold text-white">{projects[0].name}</p>
                    </div>
                </div>
            </div>
        );
