...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, size = 'md', footer, preventCloseOnBackdrop = false, noPadding = false }) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 animate-fade-in" onClick={handleBackdrop} role="dialog" aria-modal="true">
            <div className={`${containerSize} bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl shadow-2xl border border-white/20 dark:border-slate-700/50 ${size === 'full' ? 'h-[calc(100vh-8rem)] my-16' : 'rounded-2xl max-h-[calc(100vh-8rem)] my-16'} flex flex-col transform transition-all animate-scale-up`} onClick={e => e.stopPropagation()}>
                {title && (
                    <div className="px-4 sm:px-6 py-4 border-b border-slate-100 dark:border-slate-700/50 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50 rounded-t-2xl flex-shrink-0">
                        ...
                        <button type="button" onClick={onClose} className="p-2 rounded-full bg-slate-100 text-slate-600 hover:bg-rose-100 hover:text-rose-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-rose-500/20 transition-all duration-200 shadow-sm">
