...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, size = 'md', footer, preventCloseOnBackdrop = false, noPadding = false, topOffset = 'pt-20' }) => {
    ...
    const containerSize = isFull ? sizeClasses.full : `w-full ${sizeClasses[size]} mx-4`;

    return (
        <div
            ref={overlayRef}
            className={`fixed inset-0 z-[60] bg-slate-900/75 backdrop-blur-md flex ${isFull ? 'items-center justify-center p-2' : `items-start justify-center ${topOffset} pb-8`} overflow-y-auto`}
        ...
        >
            <div
                ref={modalRef}
                className={`${containerSize} backdrop-blur-2xl bg-gradient-to-br from-white/15 to-white/5 shadow-[0_8px_32px_0_rgba(0,0,0,0.4)] border border-white/20 ${isFull ? 'h-[calc(100vh-1rem)]' : 'rounded-3xl max-h-[calc(100vh-10rem)]'} flex flex-col transform transition-all overflow-hidden`}
