...
// FIX: Added useTheme import for theme toggling functionality.
...
const Header: React.FC<{
...
}> = ({ activePage, interfaceMode, setInterfaceMode, setActivePage }) => {
    const { currentUser, logout } = useAuth();
    // FIX: Added theme context to enable dark mode switching.
    ...
    }, [activePage, interfaceMode, currentUser]);
    ...
    useEffect(() => {
        ...
        // FIX: Ensured SpeechRecognition API is safely accessed to prevent errors in unsupported browsers.
    ...
    const MicrophoneIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-14 0m7 10v-10m0 10c-3.866 0-7-3.134-7-7V7a7 7 0 0114 0v4c0 3.866-3.134 7-7 7z" /></svg>;
    const BriefcaseIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>;
    ...
    return (
        <header className="relative z-10 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shadow-sm no-print transition-colors duration-300">
            <div className="h-20 flex items-center justify-between px-6 mx-auto">
                <div className="flex items-center gap-6">
                    {currentUser?.role === 'Admin' && (
                        <div className="flex items-center bg-slate-200 dark:bg-slate-900 p-1 rounded-full">
                            <button 
                    ...
                    <span className="text-sm font-semibold text-slate-600 dark:text-slate-300 hidden md:block">{pageName}</span>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="relative" ref={searchRef}>
                        ...
                        <input
                            ...
                            className="w-full md:w-80 pr-10 pl-10 py-2 border border-slate-300 dark:border-slate-600 rounded-full bg-slate-50 dark:bg-slate-700 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-slate-900 dark:text-slate-100 placeholder:text-slate-500 dark:placeholder:text-slate-400 transition-colors duration-300"
                        />
                    ...
                    <button onClick={toggleTheme} className="text-slate-500 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors duration-200 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700">
                        {theme === 'light' ? <MoonIcon /> : <SunIcon />}
                    </button>
                    
                    <button className="text-slate-500 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors duration-200 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700">
                        <BellIcon />
                    </button>

                    <div className="relative" ref={userMenuRef}>
                        <button onClick={() => setIsUserMenuVisible(prev => !prev)} className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-cover bg-center rounded-full" style={{ backgroundImage: "url('https://picsum.photos/100')" }}></div>
