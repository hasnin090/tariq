...
const AddInstallmentModal: React.FC<{
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center p-4 pt-20 animate-drawer-overlay-show" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg animate-fade-in-scale-up my-16" onClick={e => e.stopPropagation()}>
...
const DeferredPaymentPanel: React.FC<{
    ...
    const [initialPayment, setInitialPayment] = useState({ amount: 0, accountId: accounts.length > 0 ? accounts[0].id : ''});
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.description || !formData.projectId || formData.totalAmount <= 0) {
            addToast('يرجى ملء حقول الوصف والمشروع والمبلغ الإجمالي بشكل صحيح.', 'error');
            return;
        }
        if (initialPayment.amount > 0 && !initialPayment.accountId) {
            addToast('يرجى اختيار حساب لتسديد الدفعة الأولية منه.', 'error');
            return;
        }
        if (initialPayment.amount > formData.totalAmount) {
                            ...
                            <div className="p-4 border-t border-slate-200 dark:border-slate-700 space-y-4">
                                <h3 className="font-semibold text-slate-800 dark:text-slate-200">تسديد دفعة أولية (اختياري)</h3>
                                 <div className="grid grid-cols-2 gap-4">
                                     ...
                                     <select name="accountId" value={initialPayment.accountId} onChange={handleInitialPaymentChange} className={`${inputStyle} bg-white dark:bg-slate-700`} disabled={initialPayment.amount <= 0}>
...
const DeferredPayments: React.FC = () => {
    ...
    useEffect(() => {
        setDeferredPayments(JSON.parse(localStorage.getItem('deferredPayments') || '[]'));
    ...
    const processAndSaveInstallment = (targetPayment: DeferredPayment, accountId: string, amount: number, date: string) => {
        ...
        const generalCategory = categories.find(c => c.name === 'عام') || categories[0];
                                ...
                                return (
                                    <React.Fragment key={p.id}>
                                        <tr className="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-200">
                                            ...
                                            <td className="p-4 font-semibold text-slate-800 dark:text-slate-200">{formatCurrency(p.totalAmount)}</td>
                                            <td className="p-4 font-semibold text-emerald-600 dark:text-emerald-400">{formatCurrency(p.amountPaid)}</td>
                                            ...
                                            {currentUser?.role === 'Admin' && (
                                                <td className="p-4 whitespace-nowrap">
                                                    ...
                                                    <button onClick={() => handleOpenPanel(p)} className="text-primary-600 hover:underline font-semibold mx-4" disabled={p.amountPaid > 0}>تعديل</button>
